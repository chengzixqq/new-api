...
func TestMigrateRetiredFrontendOptionsMigratesValidValuesIdempotently(t *testing.T) {
	...
	for _, key := range []string{
		"ApiInfo", "Announcements", "FAQ", "UptimeKumaUrl", "UptimeKumaSlug",
...
func TestMigrateRetiredFrontendOptionsPreservesMalformedValuesAndContinues(t *testing.T) {
	...
	requireOptionMissing(t, db, "UptimeKumaUrl")
...
func TestMigrateRetiredFrontendOptionsKeepsAuthoritativeTargets(t *testing.T) {
	...
	for _, key := range []string{"ApiInfo", "UptimeKumaUrl", "UptimeKumaSlug", "console_setting.uptime_kuma_groups"} {
		requireOptionMissing(t, db, key)
	}
}
...
func TestMigrateRetiredFrontendOptionsPreservesNonClassicOptions(t *testing.T) {
	db := useFrontendOptionMigrationDB(t)
	options := []Option{
		{Key: retiredThemeOptionKey, Value: "custom-theme"},
		{Key: "custom.frontend.setting", Value: "keep"},
		{Key: "PoolStatusEnabled", Value: "true"},
		{Key: "POOL_STATUS_INTERNAL", Value: "retired"},
	}
	require.NoError(t, db.Create(&options).Error)

	require.NoError(t, MigrateRetiredFrontendOptions())
	assert.Equal(t, "custom-theme", requireOptionValue(t, db, retiredThemeOptionKey))
	assert.Equal(t, "keep", requireOptionValue(t, db, "custom.frontend.setting"))
	requireOptionMissing(t, db, "PoolStatusEnabled")
	requireOptionMissing(t, db, "POOL_STATUS_INTERNAL")
}
