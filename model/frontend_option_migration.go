...
func MigrateRetiredFrontendOptions() error {
	...
	if err := deleteRetiredUptimeOptions(); err != nil {
		migrationErrors = append(migrationErrors, err)
	}
...
func deleteRetiredUptimeOptions() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		retiredKeys := map[string]struct{}{
			...
			"UptimeKumaSlug":                      {},
			"PoolStatusEnabled":                   {},
			"PoolStatusIntervalSeconds":           {},
			"PoolStatusCategoryName":              {},
		}
		var options []Option
		if err := tx.Find(&options).Error; err != nil {
			return fmt.Errorf("read retired frontend options: %w", err)
		}
		for _, option := range options {
			_, retired := retiredKeys[option.Key]
			if !retired && !strings.HasPrefix(option.Key, "POOL_STATUS_") {
				continue
			}
			if err := tx.Delete(&option).Error; err != nil {
				return fmt.Errorf("delete retired frontend option %s: %w", option.Key, err)
			}
		}
		return nil
	})
