...
type UserSortOptions struct {
	SortBy                     string
	SortOrder                  string
	IncludeGroupRatioOverrides bool
}
...
func GetAllUsers(pageInfo *common.PageInfo, sortOptions ...UserSortOptions) (users []*User, total int64, err error) {
	...
	options := resolveUserSortOptions(sortOptions)
	query := options.Apply(tx.Unscoped()).Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password", "access_token")
	if !options.IncludeGroupRatioOverrides {
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	...
	err = options.Apply(query).Limit(num).Offset(startIdx).Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
