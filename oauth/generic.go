...
func (p *GenericOAuthProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken: authorization code received", p.config.Slug)
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken: auth_style=%d", p.config.Slug, authStyle)
	...
	if tokenResponse.Error != "" {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken OAuth error: %s",
			p.config.Slug, tokenResponse.Error))
...
func (p *GenericOAuthProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	logger.LogDebug(ctx, "[OAuth-Generic-%s] GetUserInfo: fetching user info", p.config.Slug)
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] GetUserInfo success: id=%s", p.config.Slug, userId)
	...
	if policyRaw != "" {
		...
		if !allowed {
			...
			logger.LogWarn(ctx, fmt.Sprintf("[OAuth-Generic-%s] access denied by policy: field=%s op=%s",
				p.config.Slug, failure.Field, failure.Op))
