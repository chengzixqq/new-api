...
export const channelFormSchema = z
  .object({
    ...
    responses_empty_output_guard: z.enum(['inherit', 'on', 'off']),
...
export function transformChannelToFormDefaults(
  ...
  let responsesEmptyOutputGuard: 'inherit' | 'on' | 'off' = 'inherit'
    ...
    try {
      ...
      taskPollingIntervalMs =
        ...
        parsed.task_polling_interval_ms <= 60000
          ...
          : undefined
      if (parsed.responses_empty_output_guard === true) {
        responsesEmptyOutputGuard = 'on'
      } else if (parsed.responses_empty_output_guard === false) {
        responsesEmptyOutputGuard = 'off'
      }
      maxRetries =
        Number.isInteger(parsed.max_retries) &&
        parsed.max_retries >= 0 &&
        parsed.max_retries <= 10
          ? parsed.max_retries
          : undefined
  ...
  return {
    ...
    responses_empty_output_guard: responsesEmptyOutputGuard,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formData.responses_empty_output_guard === 'inherit') {
    delete settingsObj.responses_empty_output_guard
  } else {
    settingsObj.responses_empty_output_guard =
      formData.responses_empty_output_guard === 'on'
  }

  if (formData.max_retries === undefined) {
    delete settingsObj.max_retries
  } else {
    settingsObj.max_retries = formData.max_retries
  }
